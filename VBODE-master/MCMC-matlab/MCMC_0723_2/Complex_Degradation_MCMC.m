function [oitdata,oizdata,oigdata,oip3data,ocdir,ocdaa] = Complex_Degradation_MCMC(itdata,izdata,igdata,ip3data,tir,dir,kir,bir,ubir,cdir,cdaa)
...
%     prior2=1;
%     for j=1:18
%         prior1 = prior1 * gampdf(cdir(j),a,1/a) *... % mean 1 with positive support
%     %TODO : value      
%     (tanh(mtan*log10(cdir(j)/cdir(13)))/2+1/2) *... % ZTL_light -GI becomes more stable
%             (tanh(mtan*log10(cdir(j)/cdir(14)))/2+1/2);
%         prior2 = prior2 * gampdf(cdpr(j),a,1/a) *... % mean 1 with positive support
%             (tanh(mtan*log10(cdpr(j)/cdpr(13)))/2+1/2) *... % ZTL_light -GI becomes more stable
%             (tanh(mtan*log10(cdpr(j)/cdpr(14)))/2+1/2);
%     end
%    
    a=7;
    ld = 100;
    ...
    prior2=1;
    for j=1:18
        if (j==13 | j== 14) % ZTL_light - GI costabilize
            prior1 = prior1 * exppdf(cdir(j),1/ld);
            prior2 = prior2 * exppdf(cdpr(j), 1/ld);
        else
            prior1 = prior1 * gampdf(cdir(j),a,1/a); % mean 1 with positive support
            prior2 = prior2 * gampdf(cdpr(j),a,1/a);  % mean 1 with positive support
        end
