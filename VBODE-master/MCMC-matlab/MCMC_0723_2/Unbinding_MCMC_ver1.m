function [oitdata,oizdata,oigdata,oip3data,oubir,oubaa] = Unbinding_MCMC_ver1(itdata,izdata,igdata,ip3data,tir,dir,kir,bir,ubir,cdir,ubaa)
...
for iii=1:length(ubir)
    ...
    for j=1:days
        ...
        [T2,C2] = ode15s('multi_degradation_ODE_v5',tspan,C2(end,:));
        if j==days
            plevel = [plevel; C2];
        end
  ...
  % ZTL_d - GI : ubir(5)
  % ZTL_l - GI : ubir(6)
% version 1 : compare only ZTL_dark - GI and  ZTL_light - GI   
    prior1= 1;
    prior2= 1;
    for j=1:9 % assume that binding is hard
        if j ~= 5 % we want ZTL_light - GI bind well
            prior1 = prior1 * (tanh(mtan*log10(ubir(j)/bir(1)))+d);
            prior2 = prior2 * (tanh(mtan*log10(ubpr(j)/bir(1)))+d);
        else % j=5
            prior1 = prior1 * (tanh(mtan*log10(bir(1)/ubir(5)))+d);
            prior2 = prior2 * (tanh(mtan*log10(bir(1)/ubpr(5)))+d);            
        end
    end

    
% % version 2 : compare ZTL_light - GI with other complex 
%     prior1=1;
%     prior2=1;
%     for i=1:9
%         prior1 = prior1 * (tanh(mtan*log10(ubir(i)/ubir(6)))/2+1/2);
%         prior2 = prior2 * (tanh(mtan*log10(ubpr(i)/ubpr(6)))/2+1/2);
%         if i ~= 5 % we want ZTL_light - GI bind well
%             prior1 = prior1 * (tanh(mtan*log10(ubir(i)/bir(1)))/2+1/2);
%             prior2 = prior2 * (tanh(mtan*log10(ubpr(i)/bir(1)))/2+1/2);
%         end
%     end
% % version 3 : version 1 + dissociation constant of ZTL_light - GI  = 10^-8
%     prior1= (tanh(mtan*log10(ubir(5)/ubir(6)))/2+1/2) / ...
%         ( abs(log10(ubir(6)/bir(1)) + 8) + 0.1);
%     prior2= (tanh(mtan*log10(ubpr(5)/ubpr(6)))/2+1/2) / ...
%         ( abs(log10(ubpr(6)/bir(1)) + 8) + 0.1);
%     for j=1:9 % assume that binding is hard
%         if j ~= 5 % we want ZTL_light - GI bind well
%             prior1 = prior1 * (tanh(mtan*log10(ubir(j)/bir(1)))/2+1/2);
%             prior2 = prior2 * (tanh(mtan*log10(ubpr(j)/bir(1)))/2+1/2);
%         end
%     end
