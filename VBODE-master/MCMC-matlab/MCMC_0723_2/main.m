...
for repeat=1:1:500
    ...
    for j=1:days
        ...
        [T1,C1] = ode15s('multi_degradation_ODE_v5',tspan,C1(end,:));
        if j==days
            plevel = [plevel; C1];
        end
    ...
    for i=1:iter
        ...
        %% Unbinding MCMC
        [itdata,izdata,igdata,ip3data,ubir,ubaa]=Unbinding_MCMC_ver1(itdata,izdata,igdata,ip3data,tir,dir,kir,bir,ubir,cdir,ubaa);
        ...
        igd=[igd,igdata];
        ip3d=[ip3d,ip3data];
        mse = [mse; [abs_error,norm(toc1p(2,:)-itdata),norm(ztlp(2,:)-izdata),norm(gip(2,:)-igdata),norm(prr3p(2,:)-ip3data)] ];
        ...
        writematrix(mse,['try_',num2str(repeat_num),'_MSE_',num2str(repeat),'.csv']);  
        
        if mod(iternum,2) == 0 % for print
            iternum
            abs_error
        end
        if iternum > 2000 & abs_error>6 %not converge
            break
        end
    end
end
