function dC = multi_degradation_ODE_v5(t,C) % change interpolation to pchip
...
dC(14) = bb*C(4)*C(5) - ubgp*C(14) - dgp_g * dg * C(14) -  dgp_p * dp * C(14);
    ...
    global toc1mrna
    outputArg1=max(makima(toc1mrna(1,:),toc1mrna(2,:),mod(inputArg1,24)),0);
    end

    %% GI mrna interpolation
    function [outputArg1] = gimrnainter(inputArg1)
    global gimrna
    outputArg1=max(makima(gimrna(1,:),gimrna(2,:),mod(inputArg1,24)), 0) ;
    end

    %% PRR3 mrna interpolation
    function [outputArg1] = prr3mrnainter(inputArg1)
    global prr3mrna
    outputArg1=max(makima(prr3mrna(1,:),prr3mrna(2,:),mod(inputArg1,24)), 0);
    end
