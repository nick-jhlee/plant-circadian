function [oitdata,oizdata,oigdata,oip3data,ocdir,ocdaa] = Complex_Degradation_MCMC_uni(itdata,izdata,igdata,ip3data,tir,dir,kir,bir,ubir,cdir,cdaa)
...
for iii=1:length(cdir)
    ...
    %     prior2=1;
    %     for j=1:18
    %         prior1 = prior1 * gampdf(cdir(j),a,1/a); % mean 1 and positive support
    %         prior2 = prior2 * gampdf(cdpr(j),a,1/a);
    %     end
    rel=1;
    for j=1:18
        rel=rel*gampdf(cdpr(j),a,1/a) /gampdf(cdir(j),a,1/a)
