function [oitdata,oizdata,oigdata,oip3data,oubir,oubaa] = Unbinding_MCMC_loguni(itdata,izdata,igdata,ip3data,tir,dir,kir,bir,ubir,cdir,ubaa)
...
%     f = prod(normpdf(toc1p(2,:)',itdata,terror)) / prod(normpdf(toc1p(2,:)',ptdata,terror) ) *...
%         prod( normpdf(ztlp(2,:)',izdata,zerror )) / prod(normpdf(ztlp(2,:)',pzdata,zerror)) *...
%         prod( normpdf(gip(2,:)',igdata,gerror) ) / prod(  normpdf(gip(2,:)',pgdata,gerror) ) *...
%          prod(normpdf(prr3p(2,:)',ip3data,p3error) ) / prod(normpdf(prr3p(2,:)',pp3data,p3error));
    
    mtan=1/4;
    
    prior1= (tanh(mtan*log10(10^6*ubir(5)/bb))) * ...   % ZTL_d - GI: unbinding = 10^-6 * binding
        (tanh(mtan*log10(10^8*ubir(6)/bb)));   % ZTL_l - GI: unbinding = 10^-8 * binding
%     (tanh(mtan*log10(ubir(1)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubir(2)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubir(3)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubir(4)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubir(5)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubir(6)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubir(7)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubir(8)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubir(9)/bir(1)))/2+1/2) * ...

    
    prior2=(tanh(mtan*log10(10^6*ubpr(5)/bb))) * ...   % ZTL_d - GI: unbinding = 10^-6 * binding
           (tanh(mtan*log10(10^8*ubpr(6)/bb)));   % ZTL_l - GI: unbinding = 10^-8 * binding
%     (tanh(mtan*log10(ubpr(1)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubpr(2)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubpr(3)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubpr(4)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubpr(5)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubpr(6)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubpr(7)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubpr(8)/bir(1)))/2+1/2) * ...
%         (tanh(mtan*log10(ubpr(9)/bir(1)))/2+1/2) * ...
